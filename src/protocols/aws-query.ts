...
export class AwsQueryHandler implements ProtocolHandler {
      ...
      if (Object.hasOwn(obj, key)) {
        ...
        if (value && typeof value === "object" && "member" in value) {
          ...
          if (Array.isArray(memberValue)) {
            processed[key] = memberValue.map((item) =>
              this.processAwsQueryArrays(item),
            );
          } else {
          ...
          if (Array.isArray(value)) {
            ...
            if (this.isPlainObject(value)) {
              ...
              entries.forEach(([mapKey, mapValue], index) => {
                params.append(
                  `${paramKey}.entry.${index + 1}.key`,
                  String(mapKey),
                );
                if (typeof mapValue === "object" && mapValue !== null) {
                  this.flattenObject(
                    mapValue,
                    `${paramKey}.entry.${index + 1}.value`,
                    params,
                  );
                } else {
                  params.append(
  ...
  private isPlainObject(_obj: any): boolean {
