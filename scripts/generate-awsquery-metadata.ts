...
import { discoverAwsQueryServices, type AwsQueryService } from "./discover-awsquery-services.ts";
...
function generateServiceMetadata(service: AwsQueryService): AwsQueryServiceMeta {
...
function writeServiceMetadata(serviceId: string, metadata: AwsQueryServiceMeta) {
...
function main() {
  ...
  if (!command) {
    console.error("Usage:");
    console.error("  bun scripts/generate-awsquery-metadata.ts [service-id]  # Generate specific service");
    console.error("  bun scripts/generate-awsquery-metadata.ts --all         # Generate all services");
    console.error("");
  ...
  if (command === "--all") {
    ...
    const targetService = services.find(s => 
      s.serviceId.toLowerCase() === command.toLowerCase()
    );
  ...
  for (const service of targetServices) {
    try {
      ...
      console.log(`     - ${Object.keys(metadata.operations).length} operations`);
...
export { generateServiceMetadata, writeServiceMetadata, generateServiceRegistry };
