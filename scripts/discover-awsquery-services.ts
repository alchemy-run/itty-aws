...
function extractServiceMetadata(model: any, modelPath: string): AwsQueryService | null {
  ...
  for (const [shapeId, shape] of Object.entries<any>(model.shapes)) {
    if (shape.type === 'service' && 
      ...
      const version = shape.traits?.['aws.protocols#awsQuery']?.version || 
...
function extractVersionFromPath(modelPath: string): string {
  ...
  const versionMatch = pathParts.find(part => /^\d{4}-\d{2}-\d{2}$/.test(part));
    ...
    for (const modelFile of modelFiles) {
      ...
      if (model && usesAwsQueryProtocol(model)) {
        ...
        if (serviceMetadata) {
          ...
          console.log(`  ✅ Found awsQuery service: ${serviceMetadata.serviceName} (${serviceMetadata.version})`);
        }
      }
    }
  }
...
function main() {
  ...
  for (const service of services) {
    ...
    console.log(`    Model Path: ${path.relative(process.cwd(), service.modelPath)}`);
